...
export const ConflictCard: React.FC<ConflictCardProps> = ({
  ...
  return (
    <div className={`overflow-hidden rounded-xl border border-amber-300 bg-amber-900/30 shadow-xs ${className}`}>
      {/* Alert Header */}
      <div className="flex items-start gap-3 border-b border-amber-200/80 px-5 py-4 bg-amber-900/40">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded bg-amber-200 px-2 py-0.5 text-[10px] font-black text-amber-300 uppercase">
              {language === "hi" ? "मानव समीक्षा अनिवार्य" : "HUMAN REVIEW MANDATORY"}
            </span>
          </div>
          <p className="mt-1 text-xs text-amber-300 leading-relaxed">
            {language === "hi"
        ...
        {conflicts.map((c) => (
          <div key={c.id} className="pt-3 first:pt-0">
            <div className="grid gap-3 sm:grid-cols-[130px_1fr] items-start">
              <span className="text-xs font-bold uppercase tracking-wider text-amber-300 bg-amber-900/40 px-2 py-1 rounded inline-block w-fit">
                {c.field}
              </span>
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-md border border-amber-200 bg-slate-900/70 px-2.5 py-1 text-xs font-bold text-slate-100 shadow-xs">
                    {language === "hi" ? "अपेक्षित: " : "Expected: "}
                    {c.expected}
                  </span>
                  <ArrowRight size={15} className="text-amber-400" />
                  <span className="rounded-md border border-amber-300 bg-amber-900/40 px-2.5 py-1 text-xs font-black text-amber-950 shadow-xs">
