...
export const EvidenceProvenancePanel: React.FC<EvidenceProvenancePanelProps> = ({
  ...
  return (
    <div className="bg-panelBg rounded-lg border border-slate-700 shadow-sm p-4 space-y-5">
      {/* Header */}
      ...
      <div className="p-3 bg-blue-900/30 border border-blue-800 rounded-lg text-xs text-blue-950 flex items-start gap-2.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-3.5 bg-slate-800/60 rounded-lg border border-slate-700 space-y-2.5 text-xs">
          <div className="flex items-center justify-between border-b border-slate-700 pb-2">
            <span className="font-bold text-cyan-300 uppercase text-[11px] flex items-center gap-1.5">
          ...
          <div className="space-y-1.5 font-mono text-[11px] text-slate-200">
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "परिसंपत्ति संदर्भ:" : "Asset Reference:"}</span>
              <span className="font-bold text-white">{asset?.image_id || (language === "hi" ? "अनिर्धारित" : "Unassigned")}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "कैप्चर स्रोत:" : "Capture Source:"}</span>
            ...
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "पैकेजिंग पहलू:" : "Packaging Facet:"}</span>
              <span className="text-slate-100">
                {language === "hi"
            ...
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "मूल फ़ाइल नाम:" : "Original Filename:"}</span>
              <span className="text-slate-100 truncate max-w-[170px]" title={asset?.original_filename}>
                {asset?.original_filename || "packaging_pdp.jpg"}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "रिज़ॉल्यूशन:" : "Resolution:"}</span>
              <span className="text-slate-100">{asset?.image_width} × {asset?.image_height} px</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "MIME प्रकार:" : "MIME Type:"}</span>
              <span className="text-slate-100">{asset?.mime_type || "image/jpeg"}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-sans text-slate-400">{language === "hi" ? "अछूता संरक्षित:" : "Preserved Untouched:"}</span>
            ...
            <div className="pt-1.5 border-t border-slate-700">
              <span className="font-sans text-slate-400 block text-[10px] uppercase">
                {language === "hi" ? "प्रामाणिक SHA-256 (बैकएंड रिकॉर्ड):" : "Canonical SHA-256 (Backend Record):"}
              </span>
              <span className="text-slate-400 block truncate text-[10px] select-all bg-slate-900/70 p-1 rounded border border-slate-700 mt-0.5" title={asset?.raw_sha256}>
        ...
        <div className="p-3.5 bg-slate-800/60 rounded-lg border border-slate-700 space-y-2.5 text-xs">
          <div className="flex items-center justify-between border-b border-slate-700 pb-2">
            <span className="font-bold text-slate-100 uppercase text-[11px] flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-blue-500" />
              {language === "hi" ? "व्युत्पन्न विश्लेषणात्मक आर्टिफैक्ट" : "Derived Analytical Artifacts"}
            </span>
            <span className="px-2 py-0.5 rounded bg-blue-900/40 text-blue-300 border border-blue-700 font-mono text-[10px] font-bold">
          ...
          <div className="space-y-1.5 text-[11px] text-slate-200">
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "प्रकाशीय गुणवत्ता द्वार:" : "Optical Quality Gate:"}</span>
              <span className={`font-mono font-bold ${asset?.quality_gate?.passed ? "text-emerald-400" : "text-amber-400"}`}>
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "मीट्रिक पैमाना गुणक:" : "Metric Scale Factor:"}</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "बहुभाषी ओसीआर टोकन:" : "Multilingual OCR Tokens:"}</span>
              <span className="font-mono text-slate-100 font-bold">
                {asset?.ocr?.total_tokens || asset?.ocr?.tokens?.length || 0} {language === "hi" ? "टोकन" : "tokens"}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "ओसीआर इंजन उद्भव:" : "OCR Engine Provenance:"}</span>
              <span className="font-mono text-[10px] text-slate-100">
                DBNet++ / PP-OCRv4 / PP-OCRv3
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "निष्कर्षित घोषणाएं:" : "Extracted Declarations:"}</span>
              <span className="font-mono text-slate-100 font-bold">
                {caseData.extracted_fields?.length || 0} {language === "hi" ? "फ़ील्ड" : "fields"}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-400">{language === "hi" ? "नियम इंजन निर्धारण:" : "Rule Engine Determinations:"}</span>
              <span className="font-mono text-slate-100 font-bold">
      ...
      <div className="border border-slate-700 rounded-lg p-3.5 bg-slate-900/70 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-700 pb-2">
          <div>
          ...
          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800/80 text-slate-200 font-bold border border-slate-700">
        ...
        {dagNodes.length === 0 ? (
          <div className="p-6 text-center text-xs text-slate-400 italic bg-slate-800/60 rounded border border-slate-700">
            {language === "hi"
          ...
          <div className="space-y-2">
            {dagNodes.map((node, index) => (
              <div
                ...
                className="p-3 rounded-lg border border-slate-700 bg-slate-800/60 text-xs font-mono space-y-1.5 hover:bg-slate-800/60 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="text-[11px] text-slate-400 truncate bg-slate-900/70 p-1.5 rounded border border-slate-700" title={node.payload_sha256}>
                  <span className="text-slate-400 select-none">SHA-256: </span>
                  <span className="text-slate-100">{node.payload_sha256}</span>
                </div>

                {node.metadata && Object.keys(node.metadata).length > 0 && (
                  <div className="text-[10px] text-slate-400 font-sans flex items-center gap-2 flex-wrap pt-0.5">
        ...
        {caseData.evidence_graph?.merkle_root && (
          <div className="p-2 bg-slate-800/80 rounded border border-slate-700 text-xs font-mono flex items-center justify-between text-[11px]">
            <span className="font-semibold text-slate-400">
              {language === "hi" ? "मर्कल उद्भव (रूट) मूल:" : "Merkle Provenance Root:"}
            </span>
            <span className="text-slate-100 truncate max-w-[260px]" title={caseData.evidence_graph.merkle_root}>
              {caseData.evidence_graph.merkle_root}
            </span>
          </div>
        )}
