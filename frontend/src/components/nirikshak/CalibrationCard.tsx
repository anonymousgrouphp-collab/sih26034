...
export const CalibrationCard: React.FC<CalibrationCardProps> = ({
  ...
  if (!c.available) {
    return (
      <div className={`card border-amber-300 bg-amber-900/30 p-5 rounded-xl ${className}`}>
        <div className="flex items-start gap-3">
          <div className="rounded-lg bg-amber-900/40 p-2.5 text-amber-400 shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="rounded bg-amber-200/80 px-2 py-0.5 text-[10px] font-black text-amber-300">
            ...
            <div className="mt-3 rounded-lg overflow-hidden border border-amber-200 bg-slate-900/70 p-1.5 shadow-2xs max-w-lg">
              <img
  ...
  return (
    <div className={`card overflow-hidden rounded-xl border border-slate-700 bg-slate-900/70 shadow-xs ${className}`}>
      ...
      <div className="flex items-center justify-between border-b border-slate-800 px-5 py-3.5 bg-slate-800/60">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-900/30 px-2.5 py-1 text-[10px] font-bold text-emerald-400 border border-emerald-800">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-5">
        <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-800">
          <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
            {language === "hi" ? "संदर्भ वस्तु" : "Reference Object"}
          </p>
          <p className="mt-1 text-xs font-bold text-slate-100 truncate" title={refObj}>
            {refObj}
          </p>
        </div>
        <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-800">
          ...
          <p className="mt-1 text-xs font-bold text-slate-100">{refLength} mm</p>
        </div>
        <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-800">
          <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
            {language === "hi" ? "मापे गए पिक्सल" : "Measured Pixels"}
          </p>
          <p className="mt-1 text-xs font-bold text-slate-100">{measuredPx} px</p>
        </div>
        <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-800">
          <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
            {language === "hi" ? "नियत पैमाना" : "Resolved Scale"}
          </p>
          <p className="mt-1 text-xs font-bold text-cyan-300">{scale.toFixed(4)} mm/px</p>
        </div>
      </div>

      {/* Math Formula Box */}
      <div className="border-t border-slate-800 bg-slate-800/60 px-5 py-3.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-semibold text-slate-400">
            ...
            <b className="text-slate-100">±{uncertainty} mm</b> (k=2, 95% CI)
          </span>
        </div>
        <div className="mt-2 rounded-md border border-slate-700 bg-slate-900/70 p-2.5 font-mono text-[11px] text-slate-200 flex flex-wrap items-center gap-2">
          <span>scale = reference_length / measured_pixels</span>
          <span className="text-slate-400">→</span>
          <span>{refLength} mm / {measuredPx} px</span>
          <span className="text-slate-400">≈</span>
          <span className="font-bold text-cyan-300">{scale.toFixed(4)} mm/pixel</span>
        </div>
      </div>
    </div>
  );
